...
- **`threads_publish_text` silently accepted conflicting attachments and useless `alt_text` on text-only posts** — added three handler-level validations that fast-fail with a clear error instead of forwarding undefined-behavior combinations to Meta: (1) `gif_id` is now mutually exclusive with `text_attachment`, `poll_options`, and `link_attachment` — a Threads post can only carry one attachment type at a time; (2) `alt_text` now requires a `gif_id`+`gif_provider` pair — on a text-only post there is no media for the label to describe, mirroring the [`ig_publish_reel` `alt_text` fix from #119](https://github.com/exileum/meta-mcp/issues/119) where the silent forward gave a false sense of accessibility compliance; (3) `poll_options` and `link_attachment` can no longer coexist on the same post. The tool description and the `alt_text` / `text_attachment` `.describe()` strings now spell out the new constraints. Per the [Threads Text Attachments](https://developers.facebook.com/docs/threads/create-posts/text-attachments/) docs, each attachment type requires `media_type=TEXT`; the docs explicitly forbid the `text_attachment ↔ poll` combination, and the same conservative interpretation now applies to GIF and link cards. Originally flagged by Gemini Code Assist on [PR #181](https://github.com/exileum/meta-mcp/pull/181) (review of [#77](https://github.com/exileum/meta-mcp/issues/77)) and split out as ([#185](https://github.com/exileum/meta-mcp/issues/185))
