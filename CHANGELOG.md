...
- **`threads_search_posts` `author_username` parameter verified and documented** — confirmed `author_username` is a valid `/keyword_search` query parameter per the [Threads Keyword Search docs](https://developers.facebook.com/docs/threads/keyword-search/); description updated to clarify it is a server-side filter and the username must be an exact match without `@`; added auto-strip of a leading `@` so callers can pass either form, plus rejection of empty / `@`-only inputs ([#116](https://github.com/exileum/meta-mcp/issues/116))
