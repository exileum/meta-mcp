...
### Added
- **`collaborators` parameter on Instagram publishing tools** — `ig_publish_photo`, `ig_publish_video`, `ig_publish_reel`, and `ig_publish_carousel` now accept an optional `collaborators` array (up to 3 Instagram usernames) to invite collaborators when creating the post; matches the [Instagram Graph API media reference](https://developers.facebook.com/docs/instagram-platform/instagram-graph-api/reference/ig-user/media/) which supports collaborators for Feed image, Reels, and Carousels (Stories not supported, so `ig_publish_story` is unchanged); for `ig_publish_carousel` the param is attached to the carousel parent container, not the children; leading `@` characters and whitespace in usernames are auto-stripped (same normalization as `author_username` in `threads_search_posts` from [#116](https://github.com/exileum/meta-mcp/issues/116)); pairs with the existing read-side tools (`ig_get_collaboration_invites`, `ig_respond_collaboration_invite`) ([#96](https://github.com/exileum/meta-mcp/issues/96))
