...
- **`.gitignore` now ignores `.vscode/` and the full `.env.*` glob** — `.vscode/` was previously tracked (the toptal-generated template only covered `.vscode-test`), and only specific `.env` variants like `.env.local` / `.env.production.local` were ignored, leaving `.env.production`, `.env.staging`, and `.env.development` (without the `.local` suffix) free to be committed; the existing `!.env.example` whitelist now becomes a real, load-bearing rule that keeps the example file tracked while the broader glob blocks accidental secret commits ([#106](https://github.com/exileum/meta-mcp/issues/106))
