...
- **Resource URIs migrated to namespaced `meta-mcp://` scheme** — `instagram://profile` and `threads://profile` resources renamed to `meta-mcp://instagram/profile` and `meta-mcp://threads/profile`; the previous schemes collide with the deep-link URI schemes registered by the Instagram and Threads mobile apps (e.g., `instagram://user?username=foo`), per the [MCP resources spec](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) which requires RFC 3986 compliance and recommends namespaced server-specific schemes. **Breaking change:** clients that hardcode resource URIs must update; clients that discover resources via `resources/list` are unaffected ([#110](https://github.com/exileum/meta-mcp/issues/110))
